...
% Algebra
uiAlg = uimenu(par.cMenuHandle,'Label','Algebra',...
    'Separator','on');
uimenu(uiAlg,'Label','Sum' ,...
    'callback',@(src,evnt)algCb(par.figureHandle,'sum'));
uimenu(uiAlg,'Label','Diff' ,...
    'callback',@(src,evnt)algCb(par.figureHandle,'diff'));
uimenu(uiAlg,'Label','Mean' ,...
    'callback',@(src,evnt)algCb(par.figureHandle,'mean'));
uimenu(uiAlg,'Label','Min' ,...
    'callback',@(src,evnt)algCb(par.figureHandle,'min'));
uimenu(uiAlg,'Label','Max' ,...
    'callback',@(src,evnt)algCb(par.figureHandle,'max'));
uimenu(uiAlg,'Label','./' ,...
    'callback',@(src,evnt)algCb(par.figureHandle,'rdivide'));
uimenu(uiAlg,'Label','.*' ,...
    'callback',@(src,evnt)algCb(par.figureHandle,'times'));

uimenu(uiAlg,'Label','Difference to last selected' ,...
...
uimenu(sub1,'Label','Add appendix' ,...
    'callback',@(src,evnt)changeAllTitles(par.figureHandle,'end'));
uimenu(sub1,'Label','Add prefix' ,...
...
uimenu(par.cMenuHandle,'Label','Change image selection' ,...
...
if ~verLessThan('matlab','8.4.0')
    % I honestly have no idea why this is apparently sometimes necessary 
    % with matlab 2014b... :-/
    set(par.tableHandle,'Position',[0,0,1,1]);
end
...
if(par.workOnGlobalArray)
    ...
    for i = 1 : N
        figureNumbers(i) = objs(i).getFigureNumber();
    end
...
for i = 1 : noObjs
    ...
    % cell2: 'Fig'
    Data{i,2} = num2str(objs(i).getFigureNumber);
...
function [selObjs, lastSelObj] = getSelectedObjects(fh)
par   = get(fh,'UserData');
...
selObjs = par.objs(sel);

% get the object which has been selected at last
if nargout > 1    
    lastSelObj = par.objs(par.lastSelectedRow);
end
...
for i = 1 : length(selObjs)
    ...
    initName = arrShow.removeSpecialCharsFromString(initName);
...
asObjs = getSelectedObjects(fh);

for i = 1 : length(asObjs)
    asObjs(i).minimizeFigure;
end
...
% get selected objects
[selObjs, refObj]  = getSelectedObjects(fh);

% treat the last selected object as reference
asLineup([refObj,selObjs]);
...
% treat the last selected object as reference
asMultiDiffMaps([refObj, selObjs]);
...
function algCb(fh, operation)

% get selected objects
selObjs  = getSelectedObjects(fh);

% treat the last selected object as reference
asAlgebra(selObjs, operation);
...
fid = fopen(fi1,'wt');
fprintf(fid,'Figure: %d\n\n',obj1.getFigureNumber());
fclose(fid);
fid = fopen(fi2,'wt');
fprintf(fid,'Figure: %d\n\n',obj2.getFigureNumber());
fclose(fid);
