function asLineup(allObjs, M, N, reorder, startTopLeft)
% arranges arrayShow windows in an M x N order.
%
% allObjs: if allObjs is an array of arrayShow objects,
%          allObjs(1) is used as the reference. All other
%          objects will be set to the same figure size as allObjs(1) and 
%          arranged in an M x N order.
%          
% allObjs = []:
%          global asObjs array will be used instead
%
% reorder: 
%         false: The asObjs will be arranged according to their appearance
%                in the allObjs vector.
%         true : The asObj with top-leftmost figure position will
%                used as a reference and other windows arranged according to their
%                previous figure position 
%
%

if nargin == 0 || isempty(allObjs)
    global asObjs;
    allObjs = asObjs;
end
if nargin < 5
...
%         b0 = scrPos(1,4) - h0;
b0 = scrPos(1,4) - h0;
        refObj.setFigureOuterPosition([l0, b0, w0,h0]);
    end
