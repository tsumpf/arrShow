...
% settings --------------
STAT_Y = 1;             % static y scale, 0: MatLab auto scaling
STAT_Y_MODE = 4;        % 1 scale from image statistic object
                        % 2 scale from global min an max
                        % 3 manual, fixed (use MANUAL_YLIM)
                        % 4 use windowing settings
...
% check if both colDims are selected
colDims = sort(asObj.selection.getColonDims);
if (any(colDims == 0))
    fprintf('Both colon dimensions have to be defined for plotAlongDim\n');    
    return;
end
...
if isempty(asObj.UserData) ||...
        ~isfield(asObj.UserData,'dimPlotFigHandle') ||...
        ~ishandle(asObj.UserData.dimPlotFigHandle)||...
        ~strcmp(get(asObj.UserData.dimPlotFigHandle,'Tag'),'asDimPlotFig')
    
    % create plot window below asObject    
    figPos = asObj.getFigureOuterPosition;
    figPos(2) = figPos(2) - figPos(4) - 26;
    asObj.UserData.dimPlotFigHandle = figure('OuterPosition',figPos,...
        'MenuBar','figure',...
        'ToolBar','none',...
        'Tag','asDimPlotFig',...
        'name',['DimPlot: ',asObj.getFigureTitle],...
        'IntegerHandle','off',...
        'CloseRequestFcn',@(src, evnt)closeReqCb(src,asObj));    
    asObj.UserData.dimPlotAxisHandle = axes('parent',asObj.UserData.dimPlotFigHandle);
...
ah = asObj.UserData.dimPlotAxisHandle;
...
if STAT_Y
    switch(STAT_Y_MODE)
        ...
        case 3
            YLIM = MANUAL_YLIM;
        case 4
            cw=asObj.window.getCW;
            YLIM = [cw(1)-cw(2)/2 cw(1)+cw(2)/2];
    end
    ylim(ah,YLIM);            
end
